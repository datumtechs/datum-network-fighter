...
do 
    ...
    nohup /home/juzix/work/Metis-Fighter/env_py37/bin/python main.py $cfg --bind_ip=${ip} --port=${port} --via_svc=${ip}:${via_port} --schedule_svc=${ip}:${schedule_port} > ${data_svc_log}/data_svc_${port}.log 2>&1 &
    via_port=$[${via_port}+1]
...
do 
    ...
    nohup /home/juzix/work/Metis-Fighter/env_py37/bin/python main.py $cfg --bind_ip=${ip} --port=${port} --via_svc=${ip}:${via_port} --schedule_svc=${ip}:${schedule_port} > ${compute_svc_log}/compute_svc_${port}.log 2>&1 &
    via_port=$[${via_port}+1]
    schedule_port=$[${schedule_port}+1]
done
...
do
    ...
    PYTHONPATH="../..:../../protos/:../../common" nohup /home/juzix/work/Metis-Fighter/env_py37/bin/python main.py $cfg --bind_ip=${ip} --port=${port} > ${schedule_svc_log}/schedule_svc_${port}.log 2>&1 &
done
...
/home/juzix/work/Metis-Fighter/env_py37/bin/python main.py --config=$cfg --data_svc_ip=${ip} --data_svc_port=${data_svc_base_port}
