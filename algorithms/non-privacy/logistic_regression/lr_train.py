...
class LRTrain(BaseAlgorithm):
    ...
    def parse_algo_cfg(self, cfg_dict):
        ...
        self.activation = hyperparams.get("activation", "softmax")
    ...
    def check_parameters(self):
        ...
        assert self.activation in ["sigmoid", "softmax"], f"activation can only be sigmoid,softmax. not {self.activation}"
    ...
    def compute(self, usecols_file):
        ...
        Y = tf.placeholder(tf.float64, [None, class_num], name='Y')
        W = tf.Variable(self.init_method([feature_num, class_num], dtype=tf.float64), name='W')
        logits = tf.matmul(X, W)
        if self.use_intercept:
            b = tf.Variable(self.init_method([class_num], dtype=tf.float64), name='b')
            logits = logits + b
        if self.activation == 'sigmoid':
