...
class KMeansPredict(BaseAlgorithm):
    '''
    Plaintext Kmeans predict.
    '''
    ...
    def compute(self, usecols_file, model_path):
        ...
        Y_result = pd.DataFrame(Y_pred, columns=["Y_pred_cluster"])
        Y_result.to_csv(self.output_file, header=True, index=False, float_format = '%.6f')


@ErrorTraceback("non-privacy_kmeans_predict")
def main(io_channel, cfg_dict: dict, data_party: list, compute_party: list, result_party: list, results_dir: str, **kwargs):
    '''
    This is the entrance to this module
    '''
    kmeans = KMeansPredict(io_channel, cfg_dict, data_party, compute_party, result_party, results_dir)
    result_path, result_type = kmeans.predict()
    return result_path, result_type
