...
def createSSHClient(server, port, user, password, pkey_file=None):
    if pkey_file:
    ...
    client.connect(server, port, user, password, pkey)
    return client
...
def pack_src():
    ...
    cmd = f'tar -czPf {src_zip_name} common protos data_svc compute_svc third_party/via_svc gateway'
    print(cmd)
    ret = os.system(cmd)
    succ = not bool(ret)
...
def transfer_file(scp, a_file, to_dir):
...
def update_svc_cfg(scp, remote_dir, cfg, svc_type):
    ...
    for k in dir_key:
        if k not in new_cfg.keys():
            continue
        new_dir = new_cfg[k]
        # when it is relative path, transfer to absolute path
...
def kill_all(node_cfg):
    ...
    for cfg in node_cfg:
        ...
        user, password, pkey_file = cfg['user'], cfg.get('passwd'), cfg.get('pkey_file')
        print(server, port, user, password, pkey_file)

        with createSSHClient(server, port, user, password, pkey_file) as ssh:
    ...
    for cfg in node_cfg:
        ...
        with createSSHClient(server, port, user, password, pkey_file) as ssh:
            with SCPClient(ssh.get_transport()) as scp:
                ...
                if svc_type == 'via_svc':
                    start_via_svc(ssh, remote_dir, rpc_port, use_ssl)
                else:
                    start_svc(ssh, remote_dir, svc_type, f'config_{rpc_port}.yaml')

def _clean_dir(ssh):
    print(f'clean all directory.')
    cmd = f'cd {remote_dir} && rm -rf common protos data_svc compute_svc third_party/via_svc gateway'
    print(cmd)
    ssh.exec_command(cmd)

def clean_all_directory(node_cfg):
    node_cfg = parse_cfg(node_cfg)
    one_time_ops = {cfg['host']: False for cfg in node_cfg}
    for cfg in node_cfg:
        server, port = cfg['host'], cfg['port']
    ...
    for cfg in node_cfg:
        ...
        with createSSHClient(server, port, user, password, pkey_file) as ssh:
            with SCPClient(ssh.get_transport()) as scp:
                ...
                if not one_time_ops[server]:
                    one_time_ops[server] = True
                    if env_zip and (svc_type != 'via_svc'):
                        transfer_file(scp, env_zip, remote_dir)
                        unzip(ssh, f'{remote_dir}/{os.path.basename(env_zip)}', remote_dir)
                    if src_zip:
                        transfer_file(scp, src_zip, remote_dir)
                        unzip(ssh, f'{remote_dir}/{os.path.basename(src_zip)}', remote_dir)
                    if certs_zip:
                        transfer_file(scp, certs_zip, remote_dir)
